...
- 浏览器、系统服务或后台应用可能产生额外流量，所以建议先用 Wi-Fi 打开并加载完页面，再关闭 Wi-Fi、限制其他应用联网，最后只让下载动作走移动数据。
...
1. 先保持 Wi-Fi 连接，在手机浏览器里打开部署后的网页，并等页面完全加载出来。
2. 不要点击下载按钮；先关闭 Wi-Fi。
3. 打开移动数据，并确认当前浏览器会使用 giffgaff SIM 的移动数据。
4. 临时限制其他应用后台联网，避免额外流量。
5. 回到已经打开的网页，勾选三项确认后点击下载按钮。这样通常只有 `payload.bin` 下载会走移动数据。
...
- 可以用 Wi-Fi 先打开页面，但点击下载按钮前必须关闭 Wi-Fi；Wi-Fi 下载不会产生 giffgaff 移动数据连接。
